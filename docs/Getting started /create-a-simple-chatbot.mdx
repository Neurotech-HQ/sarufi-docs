...
Its good to use virtual environment. So, I am going to create a virtual environment, activate it  and install sarufi.
...
### Let's start by creating an empty chatbot
...
:::note
For python SDK use `create_bot()` method to create a chatbot, for JavaScript SDK use `createBot()` method to create an empty chatbot and for Golang use `CreateBot()`. For Python SDK a 'Bot' object is returned. For JavaScript and Golang SDK, a JSON object is returned.
:::
...
You can either use `sarufi.chat()` method or `chatbot.respond()` to start a conversation with your chatbot.

With `sarufi.chat()` method, you need to pass the `bot_id`, `chat_id` and the `message`.

With `chatbot.respond()` method, you need to pass the `message` and `chat_id` only.
