...
:::note
For python SDK, use `create_bot()` method to create an chatbot. For JavaScript SDK, use `createBot()` method to create an empty chatbot. For Python SDK a 'Bot' object is returned. For JavaScript SDK, a JSON object is returned.
:::
...
The `chat()` method takes a string as an input and returns a JSON object as an output. The JSON object contains `message` field which contains the response from our chatbot.
...
```js 
> let chatbot = await sarufi.getBot({id: 45})
