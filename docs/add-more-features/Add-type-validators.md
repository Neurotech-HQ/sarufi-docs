...
For example, you want the bot to receive a phone number for Pizza delivery or you want a user provide an email address. Thats all **Type validation** has to do.
...
Lets update our `flows.yaml` file by adding fews lines. Note having message enclosed in single or double quatations if they contain some special characters.
...
For length validator, we shall have some additional information to add. Here is an example for yaml and json file
